...
##  config nova api
###
openstack-config --set /etc/nova/nova.conf DEFAULT osapi_compute_listen "$(hostname -s)-internalapi"
openstack-config --set /etc/nova/nova.conf DEFAULT metadata_listen "$(hostname -s)-internalapi"
...
openstack-config --set /etc/nova/nova.conf DEFAULT osapi_volume_listen "$(hostname -s)-internalapi"
openstack-config --set /etc/nova/nova.conf DEFAULT novncproxy_host "$(hostname -s)-internalapi"
### config api_database
openstack-config --set /etc/nova/nova.conf api_database connection "mysql+pymysql://nova_api:teamsun@control_vip/nova_api"
### config database
openstack-config --set /etc/nova/nova.conf database connection mysql+pymysql://nova:teamsun@control_vip/nova
### config glance
openstack-config --set /etc/nova/nova.conf glance api_servers http://control_vip:9292
###  config authtoken
openstack-config --set /etc/nova/nova.conf keystone_authtoken auth_uri http://control_vip:5000/v2.0
openstack-config --set /etc/nova/nova.conf keystone_authtoken identity_uri http://control_vip:35357
#del#openstack-config --set /etc/nova/nova.conf keystone_authtoken memcached_servers  controller1-internalapi:11211,controller2-internalapi:11211,controller3-internalapi:11211
...
openstack-config --set /etc/nova/nova.conf cache memcache_servers controller1-internalapi:11211,controller2-internalapi:11211,controller3-internalapi:11211
