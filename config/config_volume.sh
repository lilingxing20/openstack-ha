...
openstack-config --set /etc/cinder/cinder.conf DEFAULT glance_host control_vip
#openstack-config --set /etc/cinder/cinder.conf DEFAULT glance_api_servers  http://control_vip:9292
...
#openstack-config --set /etc/cinder/cinder.conf lvm  iscsi_ip_address  "$(hostname -s)-internalapi"
