...
from nova_compute_agent.openstack.common import eventlet_backdoor
...
from nova_compute_agent.openstack.common import threadgroup
rpc = importutils.try_import('nova_compute_agent.rpc')
CONF = cfg.CONF
...
class ProcessLauncher(object):
    ...
    def _wait_child(self):
        ...
        if os.WIFSIGNALED(status):
            ...
            LOG.info(_('Child %(pid)d killed by signal %(sig)d'), dict(pid=pid, sig=sig))
        else:
            code = os.WEXITSTATUS(status)
            LOG.info(_('Child %(pid)s exited with status %(code)d'), dict(pid=pid, code=code))
