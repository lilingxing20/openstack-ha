...
from nova_compute_agent.openstack.common import strutils
...
class ProcessExecutionError(Exception):
    ...
    def __init__(self, stdout=None, stderr=None, exit_code=None, cmd=None, description=None):
        ...
        if exit_code is None:
            exit_code = '-'
        message = _('%(description)s\nCommand: %(cmd)s\nExit code: %(exit_code)s\nStdout: %(stdout)r\nStderr: %(stderr)r') % {'description': description,'cmd': cmd,
           'exit_code': exit_code,
           'stdout': stdout,
           'stderr': stderr
           }
    ...
    if isinstance(check_exit_code, bool):
        ...
        if isinstance(check_exit_code, int):
            check_exit_code = [
             check_exit_code]
        if kwargs:
            raise UnknownArgumentError(_('Got unknown keyword args to utils.execute: %r') % kwargs)
